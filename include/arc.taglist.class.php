...
class TagList
{
     ...
     * @return    void
     */
    function __construct($keyword, $templet, $iswap=0)
    {
        global $dsql;
		...
		$this->Fields['title'] = $this->TagInfos['tag'];
    ...
    function CountRecord()
    {
        ...
        if($this->TotalResult==-1)
        {
            ...
            //更新浏览量和记录数
            $upquery = "UPDATE `#@__tagindex` SET total='{$row['dd']}',count=count+1,weekcc=weekcc+1,monthcc=monthcc+1 WHERE id = '{$this->Tag}' ";
            ...
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET weekcc=0,weekup='{$ntime}' WHERE id = '{$this->Tag}' ");
            }

            //月统计
            if(ceil( ($ntime - $this->TagInfos['monthup'])/$oneday ) > 30)
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET monthcc=0,monthup='{$ntime}' WHERE id = '{$this->Tag}' ");
            }
        }
        ...
        for($i=0; $i<$row; $i++)
        {
            ...
            for($j=0; $j<$col; $j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    {
                        $row['litpic'] = $GLOBALS['qiniuurl'].'/article/images/defaultpic.gif';
                    }
    ...
    function GetPageListDM($list_len,$listitem="info,index,end,pre,next,pageno")
    {
        ...
        if($totalpage <= 1 && $this->TotalResult > 0)
        {
            ...
            return '<li><span class="pageinfo">共 <strong>1</strong>页<strong>'.$this->TotalResult.'</strong>条</span></li>';
        }
        if($this->TotalResult == 0)
        {
            //return "<span class=\"pageinfo\">共0页/".$this->TotalResult."条</span>";
            return '<li><span class="pageinfo">共 <strong>0</strong>页<strong>'.$this->TotalResult.'</strong>条</span></li>';
        }
...
//        $purl = $this->GetCurUrl();
        ...
        if($this->iswap){
            $purl = '/article/wap/tags/list_'.urlencode($this->Tag);
        }else{
            $purl = '/article/pc/tags/list_'.urlencode($this->Tag);
        }

        //获得上一页和下一页的链接
        if($this->PageNo != 1)
        {
            $prepage.="<li><a href='".$purl."_$prepagenum.html'>上一页</a></li>\r\n";
            $indexpage="<li><a href='".$purl."_1.html'>首页</a></li>\r\n";
        }
        else
        {
        ...
        {
            $nextpage.="<li><a href='".$purl."_$nextpagenum.html'>下一页</a></li>\r\n";
            $endpage="<li><a href='".$purl."_$totalpage.html'>末页</a></li>\r\n";
        }
        else
        {
