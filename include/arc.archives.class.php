...
class Archives
{
     ...
     * @return    string
     */
    function __construct($aid, $tid=0)
    {
        global $dsql,$ftp;
        $this->IsError = FALSE;
        $this->ArcID = $aid;
        ...
        else
        {
            ...
            {
                // 如果当前文档不是系统模型,为单表模型
                $query = "SELECT arc.*,tp.reid,tp.typedir,tp.channeltemp,ch.addtable
                FROM `#@__archives` arc
                ...
                if($tid){
                    $querytype = "SELECT * FROM `#@__arctype` WHERE id=".$tid;
                    $typearr = $this->dsql->GetOne($querytype);
                    $this->Fields['reid'] = $typearr['reid'];
                    $this->Fields['typedir'] = $typearr['typedir'];
                    $this->Fields['channeltemp'] = $typearr['channeltemp'];
                }
            }
            else
            {
    ...
    function ParAddTable()
    {
        ...
        && isset($this->Fields[$this->SplitPageField]))
        {
            ...
            $this->TotalPage = count($this->SplitFields)-1==0?1:count($this->SplitFields)-1;
        ...
        $this->Fields['wapbody'] = str_replace('<p>#p#</p>', '', $this->Fields['body']);
    }
     ...
     * @param string $tpl 渠道名（wap， youku， 360）
     * @return    string
     */
    function MakeHtml($isremote=0, $tpl='pc')
    {
        ...
        $filename = GetFileNewName(
            ...
            $this->TypeLink->TypeInfos['namerule'],$cachedir.$this->TypeLink->TypeInfos['typedir'],$this->Fields['money'],$this->Fields['filename']
        );
        ...
        else
        {
            for($i=1;$i<=$this->TotalPage;$i++)
            {
                ...
                $this->dtp->SaveTo($TRUEfilename, $tpl);
                //如果启用远程发布则需要进行判断
		...
		if($this->TotalPage > 1) {
			//用正则匹配把分页符去掉
			$count = preg_match_all('/#p#(.*)#e#/U', $this->Fields['body2'], $match);
			foreach($match[0] as $key=>$val){
				$this->Fields['body2'] = str_replace($val, '<a name="pageanchor'.($key+1).'">'.$match[1][$key].'</a>',$this->Fields['body2']);
			}
    ...
    {
        $TRUEpath = $GLOBALS["cfg_cachedir"];//$GLOBALS["cfg_basedir"];
        return $TRUEpath;
    }
     ...
     * @return    void
     */
    function LoadTemplet($tpl = 'pc')
    {
        if($this->TempSource=='')
        {
            $tempfile = $this->GetTempletFile();
            if($tpl != 'pc'){
                $tempfile = str_replace('default', $tpl, $tempfile);
            }
        ...
        if(is_array($this->dtp->CTags))
        {
            foreach($this->dtp->CTags as $i=>$ctag)
            {
                ...
                else if($ctag->GetName()=='pagebreak')
                {
                    ...
                    else if($ctag->GetAtt('type') == 'wap'){
                        $this->dtp->Assign($i,$this->GetPagebreakWap($this->TotalPage,$this->NowPage,$this->ArcID));
                    }else{
    ...
    function GetPreNext($gtype='')
    {
        ...
        {
            $aid = $this->ArcID;
            $preR =  $this->dsql->GetOne("Select id From `#@__arctiny` where id<$aid And senddate<$time And arcrank>-1 And typeid='{$this->Fields['typeid']}' order by id desc");
            $nextR = $this->dsql->GetOne("Select id From `#@__arctiny` where id>$aid And senddate<$time And arcrank>-1 And typeid='{$this->Fields['typeid']}' order by id asc");
            ...
            else
            {
                ...
                $this->PreNext['preimg'] ="<img src=\"/article/templets/default/images/nophoto.jpg\" alt=\"对不起，没有上一图集了！\"/>";
            }
            if(is_array($nextRow))
            {
            ...
            else
            {
                ...
                $this->PreNext['nextimg'] ="<a href='javascript:void(0)' alt=\"\"><img src=\"/article/templets/default/images/nophoto.jpg\" alt=\"对不起，没有下一图集了！\"/></a>";
            }
        }
        if($gtype=='pre')
        {
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
       ...
       for($i=$start;$i<=$end;$i++)
        ...
        //$PageList.= "<li><a href='".$this->NameFirst."_all.".$this->ShortName."'>阅读全文</a></li>";
        return $PageList;
    }
    ...
    function GetPagebreak($totalPage, $nowPage, $aid)
    {
        ...
        $PageList = '';//"<li><a>共".$totalPage."页: </a></li>";
        //$PageList .= "<li><a href='".$this->NameFirst."'>首页</a></li>";
        $nPage = $nowPage-1;
        $lPage = $nowPage+1;
        if($nowPage==1)
        {
        ...
        {
			$PageList .= "<li><a href='".$this->NameFirst.".".$this->ShortName."'>首页</a></li>";
            if($nPage==1)
            {
        ...
        // for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                ...
                {
                    $PageList.="<li class=\"thisclass\"><a href='javascript:;'>1</a></li>";
                }
            }
            else
            {
                ...
                {
                    $PageList.="<li class=\"thisclass\"><a href='javascript:;'>{$n}</a></li>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
			...
			$PageList.= "<li><a href='".$this->NameFirst."_".$totalPage.".".$this->ShortName."'>尾页</a></li>";
        }
        else
        {
            //$PageList.= "<li><a href='#'>下一页</a></li>";
        ...
        // $PageList.= "<li><a href='".$this->NameFirst."_all.".$this->ShortName."#pageanchor".($nowPage-1)."'>阅读全文</a></li>";
        return $PageList;
    }
	...
	* 手机版静态分页
	**/
	function GetPagebreakWap($totalPage, $nowPage, $aid){
		if($totalPage==1)
        {
            return "";
        }
        ...
        $lPage = $nowPage+1;
		if($nowPage==1){
			$PageList .= "<li><a href='javascript:;' class='first'>首页</a></li>";
		}
        if($nowPage!=1){
			if($nPage==1){
				$PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."'>上一页</a></li>";
        ...
        if($nowPage!=$totalPage && $lPage <= $totalPage){
            $PageList.="<li><a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>下一页</a></li>";
        }
		if($nowPage==$totalPage){
			$PageList.= "<li><a href='javascript:;' class='last'>尾页</a></li>";
		}
        return $PageList;
	}
    ...
    function GetPageTitlesDM($styleName, $pageNo)
    {
        ...
        if($styleName=='link')
        {
            ...
            $revalue = $this->SplitTitles[$pageNo];
        }
        else
        {
    ...
    function GetPageTitlesST($styleName, $pageNo)
    {
        ...
        if($styleName=='link')
        {
        ...
        }else if($styleName=='joymeselect'){
            $revalue = '<span>您当前在：</span><div class="article-chapter-tit"><div>';
            foreach($this->SplitTitles as $k=>$v)
            {
                if($pageNo==1 && $i==1){
					$revalue .= "<span>$v</span> \r\n";
			...
			$revalue .= '</div></div>';
        }else if($styleName=='joymelink'){
            $revalue = '<div class="article-chapter-cont fn-clear">';
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
					if($pageNo==$i)
                    {
                        $revalue .= "<a href='javascript:;' class='on'>$v</a> \r\n";
                    }
                    else
                    {
                        $revalue .= "<a href='".$this->NameFirst.".".$this->ShortName."'>$v</a> \r\n";
			...
			$revalue .= '</div>';
        }else if($styleName=='mjoymelink'){
            $revalue = '<div class="page-con pag-hor-20"><h1 class="page-on">本文导航</h1><div class="page-list">';
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
					$acss = $pageNo==$i ? 'class="on"' : '';
                    $revalue .= "<a href='".$this->NameFirst.".".$this->ShortName."' ".
					$acss.">$v</a> \r\n";
                }
                else
                {
        ...
        {
            $revalue = "<select id='dedepagetitles' onchange='location.href=this.options[this.selectedIndex].value;'  class='article-chapter-tit'>\r\n";
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
                    $revalue .= "<option value='".$this->NameFirst.".".$this->ShortName."'>{$v}</option>\r\n";
                }
                else
                {
                    if($pageNo==$i)
                    {
                        $revalue .= "<option value='".$this->NameFirst."_".$i.".".$this->ShortName."' selected>{$v}</option>\r\n";
                    }
                    else
                    {
                        $revalue .= "<option value='".$this->NameFirst."_".$i.".".$this->ShortName."'>{$v}</option>\r\n";
                    }
                }
                $i++;
            }
