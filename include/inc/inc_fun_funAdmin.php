...
{
    global $cfg_dir_purview,$cfg_basedir,$cfg_ftp_mkdir,$isSafeMode,$cfg_cachedir;
    if($spath=='')
    {
        return true;
    }
    $flink = false;
    $truepath = $cfg_cachedir; //$cfg_basedir 替换
...
function SpGetEditor($fname,$fvalue,$nheight="350",$etype="Basic",$gtype="print",$isfullpage="false",$bbcode=false)
{
    ...
    {
        $html = '<script type="text/javascript" src="../include/ueditor/ueditor.config.js"></script>
...
<script id="container" name="body" type="text/plain">'.$fvalue.'</script>
