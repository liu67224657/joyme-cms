...
require_once(DEDEINC.'/simplehtml/simple_html_dom.php');
...
class DedeTagParse
{
     ...
     * @return    string
     */
    function SaveTo($filename, $tpl='pc')
    {
        $filename = str_replace($GLOBALS["cfg_basedir"], $GLOBALS["cfg_cachedir"], $filename);
        $tpl = empty($tpl) ? 'pc' : $tpl;
        $fp = @fopen($filename,"w") or die("DedeTag Engine Create File False");
        $res = $this->GetResult();
        if($tpl != 'pc'){
            $res = str_replace('/pc/', "/$tpl/", $res);
        }
        if($tpl != 'pc' && $tpl != 'rss'){
            $MD5= '5617cb756c6a9eeebf6a672115dca230';// 仅做替换换行符使用
            $res = str_replace('<br />', $MD5, nl2br($res));
            $res = str_get_html($res);
            if($res){
                 foreach($res->find('img') as $e){
                    $e->width='100%';
                    $e->height=null;
                }
                foreach($res->find('iframe') as $e){
                        $e->width='100%';
                        $e->height=null;
                }
                $res = str_replace($MD5,"\n",$res);
