...
class TypeUnit
{
    ...
    {
        $this->dsql->SetQuery("SELECT typeid,count(typeid) as dd FROM `#@__archives` WHERE arcrank <>-2 group by typeid");
        $this->dsql->Execute();
    ...
    function GetTotalArc($tid)
    {
        ...
        if(!isset($this->CatalogNums[$tid]) && count($ids)==1)
        {
            return 0;
        }
        else
    ...
    function ListAllType($channel=0,$nowdir=0)
    {
        ...
        $this->dsql->SetQuery("SELECT id,typedir,typename,ispart,sortrank,ishidden,channeltemp FROM `#@__arctype` WHERE reid=0 order by sortrank");
        $this->dsql->Execute(0);
     ...
     * @return    string
     */
    function ListAllTypeSel($channel=0,$nowdir=0,$keywords='')
    {
    	global $cfg_admin_channel, $admin_catalogs;
    	$this->dsql = $GLOBALS['dsql'];
    	...
    	$this->dsql->SetQuery("SELECT id,typedir,typename,ispart,sortrank,ishidden FROM `#@__arctype` WHERE typename like '{$keywords}%' order by sortrank");
    	$this->dsql->Execute(0);
    	while($row = $this->dsql->GetObject(0))
    	{
    		if( $cfg_admin_channel=='array' && !in_array($row->id, $admin_catalogs) )
    ...
    function LogicListAllSunType($id, $step)
    {
        global $cfg_admin_channel, $admin_catalogs;
        $fid = $id;
        $this->dsql->SetQuery("SELECT id,reid,typedir,typename,ispart,sortrank,ishidden,channeltemp FROM `#@__arctype` WHERE reid='".$id."' order by sortrank");
