 ...
 *
 * @version        $Id: rsslist.lib.php 3 9:19 2010年7月13日Z tianya $
...
<demo>
{dede:rsslist  flag='h' typeid='' row='' col='' titlelen='' infolen='' imgwidth='' imgheight='' listtype='' orderby='' keyword='' limit='0,1'}
...
function lib_rsslist( &$ctag, &$refObj )
{
    global $envs;
    ...
    // rsslist是否需要weight排序,默认为"N",如果需要排序则设置为"Y"
    ...
    else $innertext = GetSysTemplets("part_rsslist.htm");
 ...
 * @return    string
 */
function lib_rsslistDone(&$refObj, &$ctag, $typeid=0, $row=10, $col=1, $titlelen=30, $infolen=160,
        $imgwidth=120, $imgheight=90, $listtype='all', $orderby='default', $keyword='',
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
    global $dsql,$PubFields,$cfg_keyword_like,$cfg_index_cache,$_rsslistEnv,$envs,$cfg_cache_type,$cfg_digg_update;
    $row = AttDef($row,10);
    ...
    if($innertext=='') $innertext = GetSysTemplets('part_rsslist.htm');
    ...
    if($idlist=='')
    {
        ...
        if(!empty($typeid) && $typeid != 'top')
        {
            ...
            else
            {
                ...
                if($CrossID=='') $orwheres[] = ' (arc.typeid IN ('.GetSonIds($typeid).') OR arc.typeid2 like "%'.GetSonIds($typeid).'%") ';
                else $orwheres[] = ' (arc.typeid IN ('.GetSonIds($typeid).','.$CrossID.') OR arc.typeid2 like "%'.GetSonIds($typeid).','.$CrossID.'%") ';
            }
        }
    ...
    if($addfield != '' && !empty($channelid))
    {
        ...
        if(isset($row['addtable']) && trim($row['addtable']) != '')
        {
			...
			$addfieldsSql = ",tp3.body ";
            $addfieldsSqlJoin = " LEFT JOIN `#@__addonarticle` tp3 on arc.id=tp3.aid ";
		}
    }else{
		$addfieldsSql = ",tp3.body ";
		$addfieldsSqlJoin = " LEFT JOIN `#@__addonarticle` tp3 on arc.id=tp3.aid ";
    ...
    if($idlist!='' || $GLOBALS['_rsslistEnv']=='index' || $cfg_index_cache==0)
    {
        $needSaveCache = false;
    }
    else
    {
        $idlist = GetrsslistCache($taghash);
        if($idlist != '') {
            $needSaveCache = false;
        }
        //如果使用的是内容缓存，直接返回结果
    ...
    for($i=0; $i<$line; $i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
				...
				$row["body"] = preg_replace("/&[a-z]*;/", "", $row["body"]);
				$row["body"] = str_replace("o:p","p",$row["body"]);
				$host = 'http://'.$_SERVER['HTTP_HOST'];
            
				$row["body"] = preg_replace("/src=\"\/article\/uploads/", "src=\"".$host."/article/uploads", $row["body"]);
				$row["body"] = preg_replace("/width=\"[0-9]*\" height=\"[0-9]*\"/", "", $row["body"]);
                //$row["body"] = '<link href="http://lib.joyme.com/static/theme/default/css/core.css?13110801" type="text/css" rel="stylesheet" charset="utf-8" /><link href="http://lib.joyme.com/static/theme/default/css/global.css?13110801" type="text/css" rel="stylesheet" charset="utf-8" /><link href="http://article.joyme.com/article/templets/default/style/wenzhanglist.css" type="text/css" rel="stylesheet" charset="utf-8" />'.$row["body"];
            if(strlen($row['title']) > $titlelen ){
 ...
 * @return    string
 */
function GetrsslistCache($md5hash)
{
