...
$file_path_ =  str_replace('/opt/dede', '',  dirname(__FILE__));
$arr_file_path = explode('/',$file_path_);
$basepath_ =  $arr_file_path[1];
//die;
require_once('/opt/dede/'.$basepath_.'/ja/inc/inc_archives_functions.php');
	...
	for($i=1; $i <= $totalform; $i++)
	{
      ...
      $remotedir = preg_replace('/[^\/]*\.(jpg|gif|bmp|png)/', '', $remotefile);
		...
		//upapp 
			// $fu = new FileUtil();
			// $filen_=$fullfilename;	
			// $upapp_file = str_replace('uploads', 'upapp', $filen_);
			// $filen__path = dirname($upapp_file);
		
			// if (!is_dir($filen__path)){
					// $fu->copyFile($filen_, str_replace('uploads', 'upapp', $filen_),true);
					// $fu->unlinkFile($upapp_file);
			// }

			// if(in_array($imgfile_type,$cfg_photo_typenames))
			// {
				// $picname_info=getimagesize($filen_);
				// $pic_size_th = $picname_info[0];
				// $pic_size_eh = $picname_info[1];
				// if ($pic_size_th >320){
					// GetDDImageForApp('ddfirst', str_replace('/opt/dede', '', $filen_),1,$pic_size_th,$pic_size_eh);
				// }else{
					// $fu->copyFile($filen_, str_replace('uploads', 'upapp', $filen_),true);
				// }
			// }
