...
(function () {
    ...
    /**
     * 插入单张图片
	 * http://sy.joyme.com/uploads/allimg/141010/13-1410101P1570-L.png
     */
    function insertSingle() {
        var url = g("url"),
            ...
            vhSpace = g("vhSpace"),
            title = g("title"),
			huabaoimg = g("remotehuabaoimg").children,
		...
		for (var i = 0; i < huabaoimg.length; i++) {
			if(huabaoimg[i].checked && huabaoimg[i].value !=''){
				imgObj.alttype = huabaoimg[i].value;
				var pictype = imgObj.src.substr(imgObj.src.lastIndexOf(".")+1);
				imgObj.pictype = pictype;
			}
		}
        insertImage(imgObj);
    ...
    function insertBatch() {
        ...
        var imgObjs = [],
			...
			var huabaoimg = g("localhuabaoimg").children;
			var alttype='', pictype='';
			for (var i = 0; i < huabaoimg.length; i++) {
				if(huabaoimg[i].checked && huabaoimg[i].value !=''){
					alttype = huabaoimg[i].value;
					// imgObj.pictype = imgObj.src.substr(imgObj.src.lastIndexOf("."));
				}
			}
        for (var i = 0, ci; ci = imageUrls[i++];) {
			...
			tmpObj.alttype = alttype;
			var pictype = tmpObj.src.substr(tmpObj.src.lastIndexOf(".")+1);
			tmpObj.pictype = pictype;
            imgObjs.push(tmpObj);
        }
        insertImage(imgObjs);
        hideFlash();
