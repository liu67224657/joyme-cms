...
class ListView
{
    ...
    function CountRecord($tpl='pc')
    {
        ...
        $this->addSql  = " arc.senddate < $nowtime AND arc.arcrank > -1 ";
        ...
        if($cfg_list_son=='N')
        {
            ...
            else
            {
                if($this->CrossID=='') 
				{
					$this->addSql .= " AND ( (arc.typeid='".$this->TypeID."') OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2like) ";
				} else {
					if($cfg_cross_sectypeid == 'Y')
					{
						$typeid2Clike = " '%,{$this->CrossID},%' ";
						$this->addSql .= " AND ( arc.typeid IN({$this->CrossID},{$this->TypeID}) OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2like OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2Clike)";
					} else {
						$this->addSql .= " AND ( arc.typeid IN({$this->CrossID},{$this->TypeID}) OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2like)";
					}
				}
            }
        }
        else
        {
            ...
            else
            {
                if($this->CrossID=='') 
				{
					$this->addSql .= " AND ( $sonidsCon OR CONCAT(',', arc.joymearctypes, ',') like $typeid2like  ) ";
				} else {
					if($cfg_cross_sectypeid == 'Y')
					{
						$typeid2Clike = " '%,{$this->CrossID},%' ";
						$this->addSql .= " AND ( arc.typeid IN ({$sonids},{$this->CrossID}) OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2like OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2Clike) ";
					} else {
						$this->addSql .= " AND ( arc.typeid IN ({$sonids},{$this->CrossID}) OR CONCAT(',', arc.joymearctypes, ',') LIKE $typeid2like) ";
					}
...
//            $tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/list_default.htm";
    ...
    /**
     * Islander 20150722
     * 定时发布更新列表，倒叙更新，只更新前3页
     */
    public function MkHtml($tpl = 'pc'){
        global $cfg_remote_site;
        $tpl = empty($tpl) ? 'pc' : $tpl;
        if(empty($startpage)){
            $startpage = 1;
        }
        //创建封面模板文件
        ...
        }else if($this->TypeLink->TypeInfos['ispart']>0){//单独页面
            $reurl = $this->MakePartTemplets();
            return $reurl;
        }
        $this->CountRecord($tpl);
        //初步给固定值的标记赋值
        ...
        $cachedir = $tpl === 'pc' ? '' : '/'.$tpl;
        CreateDir($cachedir.'/'.MfTypedir($this->Fields['typedir']));
        $this->PageNo = $startpage = $totalpage = $this->TotalPage;
        $endpage = $this->TotalPage-$GLOBALS['upPageNum'];
        $endpage = $endpage>0 ? $endpage : 0;
        for($startpage; $this->PageNo > $endpage; $this->PageNo--){
            ...
            $makeFile = $this->GetTruePath().$cachedir.$makeFile;
            $makeFile = preg_replace("/\/{1,}/", "/", $makeFile);
            $murl = $this->GetTrueUrl($murl);
            $this->dtp->SaveTo($makeFile, $tpl);
            if($this->PageNo==$startpage){
                //如果列表启用封面文件，复制这个文件第一页
                $onlyrule = $this->GetMakeFileRule($this->Fields['id'],"list",$this->Fields['typedir'],'',$this->Fields['namerule2']);
                $onlyrule = str_replace("{page}",$totalpage,$onlyrule);
                $list_1 = $this->GetTruePath().$cachedir.$onlyrule;
                $murl = MfTypedir($this->Fields['typedir']).'/'.$this->Fields['defaultname'];
                $indexname = $this->GetTruePath().$cachedir.$murl;
     ...
     * @param     string  $tpl 渠道模板 $iswap
     * @return    string
     */
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0, $tpl='pc')
    {
        global $cfg_remote_site;
        $tpl = empty($tpl) ? 'pc' : $tpl;
        if(empty($startpage))
	...
	function NewMakeHtml($startpage=1, $makepagesize=0, $isremote=0, $tpl='pc')
    {
        global $cfg_remote_site;
        $tpl = empty($tpl) ? 'pc' : $tpl;
        if(empty($startpage))
        ...
        $this->ParseTempletsFirst();
        $totalpage = ceil($this->TotalResult/$this->PageSize);
		$startpage = $totalpage - $makepagesize + 1;
        if($totalpage==0)
        {
            $totalpage = 1;
        }
    ...
    function ParseDMFields($PageNo,$ismake=1)
    {
        ...
        foreach($this->dtp->CTags as $tagid=>$ctag)
        {
            ...
            {
                $limitstart = ($this->TotalPage-$this->PageNo) * $this->PageSize;
                $row = $this->PageSize;
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc', $noflag='')
    {
        ...
        $typeid=$this->TypeID;
//        $querytype = "SELECT * FROM `#@__arctype` WHERE id=".$typeid;
//        $typerow = $this->dsql->GetOne($querytype);
        if($row=='') $row = 10;
...
//                        $row['typeid'] = $typeid;
//                        $row['namerule'] = $typerow['namerule'];
//                        $row['typedir'] = $typerow['typedir'];
////                        $row['filename'] = $typerow['filename'];
//                    }
                    ...
                    {
                        $row['litpic'] = $GLOBALS['joymedefaultpic'];
                    }
    ...
    {
		$murl = '/article/pc'.str_replace("{cmspath}",$GLOBALS['cfg_cmspath'],$this->Fields['typedir']).'/';
        $prepage = $nextpage = '';
        ...
        {
            $prepage.="<li><a href='".$murl.str_replace("{page}",$prepagenum,$tnamerule)."'>上一页</a></li>\r\n";
            $indexpage="<li><a href='".$murl.str_replace("{page}",$totalpage,$tnamerule)."'>首页</a></li>\r\n";
        }
        else
        {
            // $indexpage="<li>首页</li>\r\n";
		...
		if($this->PageNo != 1)
        {
            $nextpage.="<li><a href='".$murl.str_replace("{page}",$nextpagenum,$tnamerule)."'>下一页</a></li>\r\n";
            $endpage="<li><a href='".$murl.str_replace("{page}",1,$tnamerule)."'>末页</a></li>\r\n";
        }
        else
        {
            // $endpage="<li>末页</li>\r\n";
		...
		$l_pageno = $totalpage - $this->PageNo +1;
        if($l_pageno >= $total_list)
        {
            $j = $l_pageno-$list_len;
            $total_list = $l_pageno+$list_len;
		...
		if($total_list>7 && $total_list-$j<7){
			$j = $total_list-6;
		}
        for($j;$j<=$total_list;$j++)
        {
            if($j==($totalpage-$this->PageNo+1))
            {
                $listdd.= "<li class=\"thisclass\">$l_pageno</li>\r\n";
            }
            else
            {
                $listdd.="<li><a href='".$murl.str_replace("{page}",($totalpage-$j+1),$tnamerule)."'>".$j."</a></li>\r\n";
            }
        }
        $plist = '';
        ...
        if(preg_match('/rep/i', $listitem)){
			$preg = '/href=\'(.*?)\'/im';
			$plist = preg_replace_callback($preg, function($m){ return 'href="'.str_replace('/','#',$m[1]).'"';}, $plist);
		}
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
		...
		if($this->PageNo!=$totalpage && $totalpage>1)
        {
            ...
            $indexpage="<li><a href='".$purl."PageNo=$totalpage'>首页</a></li>\r\n";
        }
        else
        {
		...
		if($this->PageNo != 1)
        {
            ...
            $endpage="<li><a href='".$purl."PageNo=1'>末页</a></li>\r\n";
        }
        else
        {
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==($totalpage-$this->PageNo+1))
            {
                $listdd.= "<li class=\"thisclass\"><a>$l_pageno</a></li>\r\n";
            }
            else
            {
                $listdd.="<li><a href='".$purl."PageNo=".intval($totalpage-$j+1)."'>".$j."</a></li>\r\n";
            }
        }
