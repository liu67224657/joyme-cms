...
if ( ! function_exists('GetFileUrl'))
{
    ...
    $money=0, $filename='',$moresite=0,$siteurl='',$sitepath='')
    {
        ...
        if(!preg_match("/http/", $articleUrl) && strpos($articleUrl, '/') !== 0){
            $articleUrl = '/'.$articleUrl;
        }
        if(!preg_match("/http/", $articleUrl) && strpos($articleUrl, '/article') !== 0){
            $articleUrl = '/article/pc'.$articleUrl;
        }
...
if ( ! function_exists('GetTypeUrl'))
{
    function GetTypeUrl($typeid,$typedir,$isdefault,$defaultname,$ispart,$namerule2,$moresite=0,$siteurl='',$sitepath='')
    {
        ...
        if(!preg_match("/http/", $reurl) && strpos($reurl, '/') !== 0){
            $reurl = '/'.$reurl;
        }
        if(!preg_match("/http/", $reurl) && strpos($reurl, '/article') === false){
