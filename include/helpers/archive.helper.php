...
//            $tags .= "<a href='/article/tags/".urlencode($row['tid'])."_1.html'>".$row['tag']."</a> ";
        }
        return $tags ? $tags : '无';
    }
}
...
if ( ! function_exists('GetAllTags'))
{
    ...
    {
        global $dsql;
        $query = "SELECT id,tag FROM `dede_tagindex` ORDER BY total DESC LIMIT 20;";
        $dsql->Execute('tag',$query);
        $tags = array();
        while($row = $dsql->GetArray('tag'))
        {
        ...
        */
        if($tags!='')
        {
            ...
            foreach($oldtags as $tag)
            {
                ...
                {
                    $dsql->ExecuteNoneQuery("UPDATE `#@__taglist` SET `arcrank` = '$arcrank', `typeid` = '$typeid' WHERE aid='$id' AND  tag LIKE '$tag' ");
                }
            }
        }else{
            $oldtag = GetTags($id);
