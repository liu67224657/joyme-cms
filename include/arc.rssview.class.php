...
class RssView
{
    ...
    function GetArcList($innertext="")
    {
		...
		$query = "SELECT arc.*,tp.typedir,tp.typename,tp.isdefault,
        tp.defaultname,tp.namerule,tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,t3.body
        FROM `#@__archives` arc LEFT JOIN `#@__arctype` tp ON arc.typeid=tp.id 
		LEFT JOIN `#@__addonarticle` t3 ON arc.id=t3.aid 
        WHERE $orwhere $ordersql LIMIT 0,".$this->MaxRow;
		...
		if($row["body"]=='')
        	{
        		$query_addatt = "SELECT * FROM `#@__addon17_lanmu` WHERE aid=".$row["id"]." ";
				$arr = $this->dsql->GetOne($query_addatt);						
				$row['body'] = $arr['body'];
        	}
            //处理一些特殊字段
			...
			$row["body"] = '<link type="text/css" rel="stylesheet" href="http://lib.joyme.com/static/theme/default/css/core.css?13110801" /><link type="text/css" rel="stylesheet" href="http://lib.joyme.com/static/theme/default/css/global.css?13110801" /><link type="text/css" rel="stylesheet" href="http://article.joyme.com/article/templets/default/style/wenzhanglist.css" />'.$row["body"];
