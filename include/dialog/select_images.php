...
//function ChangeImage(surl){ document.getElementById('<?php echo str_replace('name', '', substr($f, strrpos($f, '.')+1))?>view').src = surl; }
</script>
</head>
...
//function nullLink(){ return; }
//function ChangeImage(surl){ document.getElementById('floater').style.display='block';document.getElementById('<?php echo str_replace('name', '', substr($f, strrpos($f, '.')+1))?>view').src = surl; }
function TNav()
{
...
{	reimg = '<?php echo $conf['qiniu']['attachurl']?>/article'+reimg;
	...
	if(window.opener.document.<?php echo $f?> != null)
	{
		...
		if(window.opener.document.getElementById('div<?php echo $v?>'))
	    {
		 if(TNav()=='IE'){
			...
			<?php $v = str_replace('name', '', substr($f, strrpos($f, '.')+1)).'view'?>
...
$arr = array();
while($file = $dh->read()){
	$arr[$file] = filemtime("$inpath/$file");
}
arsort($arr);
