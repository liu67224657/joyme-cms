...
if($dopost=='saveedit')
{
    ...
    $setimgsql = $_FILES['avatar']['tmp_name'] != '' ? ', avatar = \''.uploadImg($_FILES['avatar']).'\'' : '';
    if($id!=1){
        $query = "UPDATE `#@__admin` SET uname='$uname',usertype='$usertype',tname='$tname',email='$email'".$setimgsql." ,intro='$intro', wikidbs='".implode(',', $wikidbs)."',typeid='$typeid' $pwd WHERE id='$id'";
    }else{
        $query = "UPDATE `#@__admin` SET uname='$uname',tname='$tname',email='$email'".$setimgsql.",intro='$intro', wikidbs='".implode(',', $wikidbs)."',typeid='$typeid' $pwd WHERE id='$id'";
    }
...
// wiki 查找
$link = mysql_connect($wikidbconf['host'], $wikidbconf['username'], $wikidbconf['password']);
$resource = mysql_query('SHOW DATABASES');
$wikidbs = array();
while($r = mysql_fetch_assoc($resource)){
	if(preg_match('/^[a-z][a-z0-9]*wiki$/', $r['Database'])){
		$wikidbs[] = str_replace('wiki', '', $r['Database']);
	}
}
