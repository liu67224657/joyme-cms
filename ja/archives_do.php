...
if($dopost=='editArchives')
{
	...
	echo "<script language='javascript' type='text/javascript'>window.location.href='".$gurl."?aid=".$aid."&t=".time()."';</script>";
	echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL='.$gurl.'?aid='.$aid."&tt=".time().'">';
    die('等待跳转');
}
/*--------------------------
//浏览文档
...
else if($dopost=="viewArchives")
{
	...
	$list_phone_jump = (isset($marticle) && $marticle==1) ? false : true;
    if($trow['issystem'] != -1 && $list_phone_jump)
    {
	...
	if(!empty($marticle)){
		$arcurl = "http://marticle.joyme.$com".str_replace('article','marticle',$arcurl);
		echo "<script language='javascript'>location.href='$arcurl';</script>";
	}else{
		if(!preg_match("/http:/", $arcurl))
		{
			$arcurl = $domain.$arcurl;
		}
...
else if($dopost=="checkArchives")
{
	...
	if(file_exists($arcgamesfilepath)){
		...
		if($arcgamesarr[$aid]){
			arcGames($arcgamesarr[$aid]);
			unset($arcgamesarr[$aid]);
...
else if($dopost=="delArchives")
{
    ...
    if($fmdo=='yes')
    {
        ...
        $qstrs = explode("`",$qstr);
        if(count($qstrs)>5){
            ShowMsg("批量操作请不要超过5个选择！",$ENV_GOBACK_URL);
            exit();
        }
        $okaids = Array();
        ...
        $data = array('archiveid'=>$aid, 'gameids'=>0, 'contenttype'=>0);
...
else if($dopost=='return')
{
    ...
    $qstrs = explode("`", $qstr);
    if(count($qstrs)>5){
        ShowMsg("批量操作请不要超过5个选择！","recycling.php");
        exit();
    }
    foreach($qstrs as $aid)
    {
...
?>
<div align='center' class='quickselfoot'>输入关键字:<input type='text' value=''  onkeyup="getTypeName(this.value);"/></div>
