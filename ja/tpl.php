...
else if($action == 'saveedit')
{
    ...
    //存入db
    $query = "SELECT * FROM `#@__tpl` WHERE tplname='".$filename."' and tpldir='".$acdir."'";
    $tplinfo = $dsql->GetOne($query);
    $tplcontent = addslashes($content);
    if(!is_array($tplinfo))
    {
    	//insert
    	$query = "INSERT INTO `#@__tpl`(tplname,tpldir,tplcontent)
    	VALUES ('$filename','$acdir','$tplcontent');";
    	
    	if(!$dsql->ExecuteNoneQuery($query))
    	{
    		ShowMsg('DEDE模板文件保存失败，可能是模版内容太多！', '-1');
    		exit();
    	}
    }else{
    	//update
    	$query = "UPDATE `#@__tpl` SET tplcontent='$tplcontent' where tplname='$filename' and tpldir='$acdir';";
...
else if ($action == 'del')
{
    ...
    $query = "Delete From `#@__tpl` where tplname='$filename' and tpldir='$acdir';";
    if($dsql->ExecuteNoneQuery($query) && unlink($truefile))
    {
...
{
	ShowMsg("模版上传功能已经关闭了哦！","javascript:;");
	exit();
...
{
	ShowMsg("修改标签碎片功能已经关闭了哦！","javascript:;");
	exit();
    if($action=='addnewtag')
    {
        $democode = '<'."?php
