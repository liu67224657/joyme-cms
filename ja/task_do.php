...
else if($dopost=='makeparenttype')
{
    ...
    $ids = GetTopids($typeid);
    if($up_othertypes){
        $arr = explode('_', $up_othertypes);
        foreach($arr as $val){
            $ids .= ','.GetTopids($val);
        }
    }
//    $topids = explode(',', GetTopids($typeid));
    ...
    $topids = array_merge($topids);
    if(empty($curpage)) $curpage = 0;
    $tid = $topids[$curpage];
//    $nowtpl = isset($nowtpl) ? $nowtpl : 'pc';
    $tplnu = isset($tplnu) ? $tplnu : 0;
    if(isset($cfg_Cs[$tid]) && $cfg_Cs[$tid][1]>0)
    {
        require_once(DEDEINC."/arc.listview.class.php");
        $lv = new ListView($tid);
        $channeltemp = explode(',', $lv->Fields['channeltemp']);
        array_unshift($channeltemp, 'pc');
        $nowtpl = isset($channeltemp[$tplnu]) ? $channeltemp[$tplnu] : 'pc';
        if($nowtpl){
            $lv->NewMakeHtml($startpage=1, $makepagesize=10,0,$nowtpl);
        }
        $lv->Close();
        $tplnu++;
        $tplnu = isset($channeltemp[$tplnu]) ? $tplnu : 0;
        $jumpurl .= '&nowtpl='.$nowtpl.'&tplnu='.$tplnu;
//        echo $jumpurl;exit;
    }
    else
    {
    ...
    if($curpage >= count($topids)-1 && $nowtpl != 'pc')
    {
        if( !empty($doposttmp) )
        {
