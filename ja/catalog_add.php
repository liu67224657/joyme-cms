...
//读取渠道信息
$channellist = array();
$channelrs = file_get_contents($channelApiUrl.'?c=source&a=channellist');
if($channelrs){
    $channellist = json_decode($channelrs,true);
}
...
else if($dopost=='save')
{
    ...
    // 查找渠道模板是否存在
    $tplarr = array($tempindex, $templist, $temparticle);
    $cfg_templets_dir = $cfg_basedir.$cfg_templets_dir;
    if(!empty($channeltemp)){
        foreach($tplarr as $tplpath){
            $tplname = substr($tplpath, strpos($tplpath, '/'));
            foreach($channeltemp as $tempdir){
                if(!file_exists($cfg_templets_dir.'/'.$tempdir.$tplname)){
                    ShowMsg($cfg_templets_dir.'/'.$tempdir.$tplname.' 渠道模板不存在',"-1");
                    exit();
                }
            }
        }
        $channeltemp = implode(',', $channeltemp);
    }else{
        $channeltemp = '';
    }
    $newchannelcids = '';
    if(!empty($channelcids)){
        foreach($channelcids as $v){
            $newchannelcids .=','.intval($v);
        }
        $newchannelcids = substr($newchannelcids,1);
    }

    $in_query = "INSERT INTO `#@__arctype`(reid,topid,sortrank,typename,typedir,isdefault,defaultname,issend,channeltype,
    tempindex,templist,temparticle,modname,namerule,namerule2,
    ispart,corank,description,keywords,seotitle,moresite,siteurl,sitepath,ishidden,`cross`,`crossid`,`content`,`smalltypes`,`channeltemp`,`channelcids`)
    VALUES('$reid','$topid','$sortrank','$typename','$typedir','$isdefault','$defaultname','$issend','$channeltype',
    '$tempindex','$templist','$temparticle','default','$namerule','$namerule2',
    '$ispart','$corank','$description','$keywords','$seotitle','$moresite','$siteurl','$sitepath','$ishidden','$cross','$crossid','$content','$smalltypes','$channeltemp','$newchannelcids')";
