...
-------------------------------*/
    ...
    $images = $_POST['images'];
    $qiniuimgs = json_decode($images, true);
    ...
    $addrow = $dsql->GetOne("SELECT * FROM `#@__joyme_arcaddtable` WHERE aid='$id'");
    if ($addrow['id']) {
    ...
    //检查旧的图片是否有更新，并保存
    //-----------------------------------------
    $tmpimgarr1 = $tmpimgarr2 = array();
    for ($i = 1; $i <= 120; $i++) {
        if (!isset(${'imgurl' . $i})) continue;
        $info = '';
        $iorder = intval(${'imgorder' . $i});
...
//        $litimgfile = $cfg_basedir.$ddurl;
        ...
        if (isset(${'imgfile' . $i}) && is_uploaded_file(${'imgfile' . $i})) {
            ...
            if (!is_array($imginfos)) {
                ...
                // $imgurls .= 
                if ($tmpimgarr1[$iorder]) {
                    $tmpimgarr2[] = "{dede:img ddimg='$ddurl' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
                } else {
                    $tmpimgarr1[$iorder] = "{dede:img ddimg='$ddurl' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
                }
                continue;
            }
            $urlInfo = parse_url($imgfile);
            $imgfilepath = ltrim($urlInfo['path'], '/');
            $imgfile = uploadImgToQiniu($tmpFile, $imgfilepath);
            $imginfos = @GetImageSize($imgfile, $info);
//            if($ddurl==$iurl)
//            {
...
//            }
            ...
            // $imgurls .= 
            if ($tmpimgarr1[$iorder]) {
                $tmpimgarr2[] = "{dede:img ddimg='$litpicname' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
            } else {
                $tmpimgarr1[$iorder] = "{dede:img ddimg='$litpicname' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
            }
        } //没上传图片(只修改msg信息)
        else {
            $iinfo = str_replace("'", "`", stripslashes(${'imgmsg' . $i}));
...
//            }
            $imginfos = @GetImageSize($ddurl);
            // $imgurls .= 
            if (isset($tmpimgarr1[$iorder])) {
                $tmpimgarr2[] = "{dede:img ddimg='$ddurl' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
            } else {
                $tmpimgarr1[$iorder] = "{dede:img ddimg='$ddurl' text='$iinfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $iurl {/dede:img}\r\n";
            }
    ...
    if ($formhtml == 1 && !empty($imagebody)) {
        ...
        // $imgurls .= 
        $tmpimgarr2[] = GetCurContentAlbum($imagebody, $copysource, $litpicname);
    ...
    if (is_array($_SESSION['bigfile_info'])) {
        foreach ($_SESSION['bigfile_info'] as $k => $v) {
            ...
            // $imgurls .= 
            $tmpimgarr2[] = "{dede:img ddimg='$litpicname' text='$imginfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $v {/dede:img}\r\n";
        }
    }

    /*---------------------
    function _getformqiniu()
    通过前台正常上传的图片
    ---------------------*/
    if (is_array($qiniuimgs)) {
        foreach ($qiniuimgs as $k => $v) {
            $imgsrc = $v['imgsrc'];
            if (strlen($imgsrc) < 2) continue;

            if ($v['isuse'] == 0) {
                $res = parse_url($imgsrc);
                delImage(substr($res['path'], 1));
            } else {
                $info = '';
                $imginfos = GetImageSize($imgsrc, $info);
                $idnum = $k + 1;
                $imginfo = !empty(${'picinfook' . $idnum}) ? ${'picinfook' . $idnum} : '';
                $iorder = !empty(${'imgorderok' . $idnum}) ? ${'imgorderok' . $idnum} : '';
                if ($tmpimgarr1[$iorder]) {
                    $tmpimgarr2[] = "{dede:img ddimg='$imgsrc' text='$imginfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $imgsrc {/dede:img}\r\n";
                } else {
                    $tmpimgarr1[$iorder] = "{dede:img ddimg='$imgsrc' text='$imginfo' width='" . $imginfos[0] . "' height='" . $imginfos[1] . "'} $imgsrc {/dede:img}\r\n";
                }
                // $imgurls .= "{dede:img ddimg='$imgsrc' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $imgsrc {/dede:img}\r\n";
                //把图片信息保存到媒体文档管理档案中
                $inquery = "
                   INSERT INTO #@__uploads(title,url,mediatype,width,height,playtime,filesize,uptime,mid)
                    VALUES ('{$title}','{$imgsrc}','1','" . $imginfos[0] . "','" . $imginfos[1] . "','0','" . filesize($imgsrc) . "','" . $ntime . "','$adminid');
                 ";
                $dsql->ExecuteNoneQuery($inquery);
                $fid = $dsql->GetLastID();
                AddMyAddon($fid, $iurl);
    ...
    ksort($tmpimgarr1);
    $tmpimgarr = array_merge($tmpimgarr1, $tmpimgarr2);
    $imgurls .= implode('', $tmpimgarr);
