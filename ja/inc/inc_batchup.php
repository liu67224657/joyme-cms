...
function DelArc($aid, $type='ON', $onlyfile=FALSE,$recycle=0)
{
    ...
    //查询表信息
    $query = "SELECT ch.maintable,ch.addtable,ch.nid,ch.issystem,tp.channeltemp FROM `#@__arctiny` arc
    ...
    $arcdata = $dsql->GetOne($query);
    if($arcdata['joymearctypes']){
        $tids = explode(',', $arcdata['joymearctypes']);
        $tids[] = $arcdata['typeid'];
    }else{
         $tids = array($arcdata['typeid']);
    }
    foreach($tids as $tid){
        $querytype = "SELECT * FROM `#@__arctype` WHERE id=".$tid;
        $typearr = $dsql->GetOne($querytype);
        $arcurl = GetFileUrl($arcRow['aid'],$tid,$arcRow['senddate'],$arcRow['title'],$arcRow['ismake'],
                       $arcRow['arcrank'],$typearr['namerule'],$typearr['typedir'],$arcRow['money'],$arcRow['filename']);
        $arcurl = str_replace('/article/pc', '', $arcurl);
        if($typearr['channeltemp']){
            $tmparr = explode(',', $typearr['channeltemp']);
            foreach($tmparr as $val){
                $htmlfilepath = GetTruePath().'/'.$val.str_replace($GLOBALS['cfg_basehost'],'',$arcurl);
                if(file_exists($htmlfilepath) && !is_dir($htmlfilepath)){
                    @unlink($htmlfilepath);
                }
            }
        }
...
//    exit;
    ...
    {
        if($row['channeltemp']){
            $tmparr = explode(',', $row['channeltemp']);
            foreach($tmparr as $val){
                $htmlfilepath = GetTruePath().'/'.$val.str_replace($GLOBALS['cfg_basehost'],'',$arcurl);
                if(file_exists($htmlfilepath) && !is_dir($htmlfilepath)){
                    @unlink($htmlfilepath);
