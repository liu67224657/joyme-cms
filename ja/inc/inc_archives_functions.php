...
function GetCurContentAlbum($body, $rfurl, &$firstdd)
{
    ...
    if(empty($cfg_ddimg_width))    $cfg_ddimg_width = 480;
    $rsimg = '';
...
{
    global $cfg_multi_site,$cfg_basehost,$cfg_basedir,$cfg_image_dir,$arcID,$cuserLogin,$dsql,$cfg_ddimg_width,$cfg_ddimg_height,$forapp_dir_path,$root_path_tag, $conf;
    $cfg_uploaddir = $cfg_image_dir;
    preg_match("/<img([^>]*)\s*src=\"data:image([^\"]+)\"[^>]+>/isU",$body,$prs);//去掉base64的图片
    if(!empty($prs)){
    	ShowMsg("上传图片有本地类型的，请先上传，不要直接粘贴!","-1");
    	exit();
    }
    ...
    $img_array = array();
    $img_array2 = array();
    preg_match_all("/<img[^>]*src=[\"|'|\s]{0,}(http:\/\/([^>]*)\.(gif|jpg|png))[^>]*>/isU",$body,$img_array);
    preg_match_all("/href=[\"|'|\s]{0,}(http:\/\/([^>]*)\.(gif|jpg|png))/isU",$body,$img_array2);
    
    $pimg_array = $img_array[0];
    

    $img_array = array_unique($img_array[1]);
    $img_array2 = array_unique($img_array2[1]);

    $img_array = array_merge($img_array,$img_array2);
    ...
    $img_resize_array = array();
    
    foreach($img_array as $key=>$value)
    {
		if(strpos($value, $conf['qiniu']['attachurl']) !== false){
			continue;
		}
        ...
        //图片压缩到 upapp  S
		/*
        $app_path = str_replace("uploads", "upapp", $imgPath);
        if(!is_dir($app_path.'/'))
        {
           $copy_img = str_replace("uploads", "upapp", $rndFileName);
           $fu->copyFile($rndFileName, $copy_img,true);
           $fu->unlinkFile($copy_img);
        }
        $copy_img = $rndFileName;
        $img_temp=pathinfo($copy_img);
        $img_info = getimagesize($copy_img);
        $oldpic = $img_temp['dirname'].'/'.$img_temp['basename'];
        $litpic = str_replace('uploads', 'upapp', $oldpic);
        if($img_info[0]>480){
        	@ImageResize($oldpic,$cfg_ddimg_width,$cfg_ddimg_height,$litpic);
        }else{
        	$fu->copyFile($oldpic, $litpic,true);
        }
       
        $autolitpic = isset($_POST['autolitpic']) ? $_POST['autolitpic'] : 0;
        if ($autolitpic == 1 && $key==0 ){
        	$fu->copyFile($litpic, str_replace('.', '-lp.', $litpic),true);
        }*/
        //图片压缩到 upapp  END
        ...
        {
			// 七牛上传图片-START
			$fileurl = $imgsrc = uploadImgToQiniu($cfg_basedir.$fileurl);
			// 七牛上传图片-END
			$info = '';
			$imginfos = GetImageSize($rndFileName, $info);
			$fsize = filesize($rndFileName);
			...
			foreach ($pimg_array as $k=>$v){
            	if(strpos($v, $value) != false){
            		if(strpos('width', $v) == false || strpos('height', $v) == false){
            			$pv = str_replace($value, $fileurl, $v);
            			$replacestr = "<img ".$imginfos[3].substr($pv,4);
            			$body = str_replace($v, $replacestr, $body);
            		}else{
            			$replacestr = str_replace($value, $fileurl, $v);
            			$body = str_replace($v, $replacestr, $body);
            		}
            	}
            }
        ...
        if ($root_path_tag == 'D:/wamp/www'){
        	$img_resize_array[] = str_replace('D:/wamp/www', '', $rndFileName);
        }else{
        	$img_resize_array[] = str_replace('/opt/dede', '', $rndFileName);
        }
    }
    $htd->Close();
    if (!empty($img_resize_array)){
    	 //文件处理类 ST
    	 //include 'FileUtil.php';
    	 $fu = new FileUtil();
    	 //文件处理类 END
    	 foreach ($img_resize_array as $k=>$v){
    	 	$picname = preg_replace("/[\"|'| ]{1,}/", '', $v);
    	 	$picname_info=getimagesize($picname);
    	 	$pic_size_th = $picname_info[0];
			$pic_size_eh = $picname_info[1];
    	 	
    	 	// $app_dir_path =  dirname(str_replace('uploads', 'upapp', $root_path_tag.$picname));
    	 	// if (!is_dir($app_dir_path)){
    	 		// $fu->copyFile($root_path_tag.$picname, str_replace('uploads', 'upapp', $root_path_tag.$picname),true);
    	 		// $fu->unlinkFile(str_replace('uploads', 'upapp', $root_path_tag.$picname));
    	 	// }
    	 	
    	 	if ($pic_size_th >480){
    	 		$litpic = GetDDImageForApp('ddfirst', $picname,1,$pic_size_th,$pic_size_eh);
    	 	}
			// else{
    	 		// $fu->copyFile($root_path_tag.$picname, str_replace('uploads', 'upapp', $root_path_tag.$picname),true);
    	 	// }
...
{
    global $envs, $typeid, $dsql;
    require_once(DEDEINC.'/arc.archives.class.php');
    if($ismakesign) $envs['makesign'] = 'yes';
    $query = "SELECT * FROM `#@__archives` WHERE `id` = '$aid'";
    $arcRow = $dsql->GetOne($query);
    if($arcRow['joymearctypes']){
        $tids = explode(',', $arcRow['joymearctypes']);
        $tids[] = $arcRow['typeid'];
        foreach($tids as $tid){
            $arc = new Archives($aid, $tid);
            $reurl = $arc->MakeHtml($isremote);
            $channeltemp = $arc->Fields['channeltemp'] != '' ? explode(',', $arc->Fields['channeltemp']) : array();
            if($channeltemp){
                foreach($channeltemp as $val){
                    $arc = new Archives($aid, $tid);
                    $arc->MakeHtml($isremote, $val);
                }
            }
        }
    }else{
...
//$forapp_dir_path = 'D:/wamp/www/dede-img-app/uploads/allimg';
//$root_path_tag = 'D:/wamp/www';
$forapp_dir_path = '/opt/www/article/alpha/uploads/allimg';
$each_forapp_dir_path = '/opt/www/article/alpha/uploads/allimg';
$root_path_tag = '/opt/www/article/alpha/';
/**
 *  取第一个图片为缩略图
 *
 * @access    public
...
{
	global $forapp_dir_path,$root_path_tag;
    $litpic = '';
...
function app_dir_file($path){
	global $forapp_dir_path,$root_path_tag;
	if(is_file($path)){
		echo 'fcn: app_dir_file ->'.$path;
	}
	if(!is_dir($path)){
		return false;
	}
	...
	while(false !== ($foldeordir = readdir($handle))){
		if($foldeordir != "." && $foldeordir != ".."){
			if(is_dir($path."/".$foldeordir)){
				$this->app_dir_file("{$path}/{$foldeordir}");
			}elseif(is_file("{$path}/{$foldeordir}")){
				$retrun[] = str_replace($root_path_tag,'',$path.$foldeordir);
			}
		}
	}
	closedir($handle);
...
function EeachDDImgForApp(){
	global $forapp_dir_path,$root_path_tag,$each_forapp_dir_path;
	$dir_path = $each_forapp_dir_path; //目录path
	$list_dir = app_list_all($dir_path);
	if(!empty($list_dir)){
		//include 'FileUtil.php';
		$fu = new FileUtil();
		foreach($list_dir as $k=>$v){
			if ($k>1){
				$new_dir = $dir_path.'/'.$v.'/';
				$list_file = app_dir_file($new_dir);
				if (!empty($list_file)){
					foreach ($list_file as $key =>$val){
						$picname = preg_replace("/[\"|'| ]{1,}/", '', $val);
						echo 'fcn: EeachDDImgForApp-->'.$picname.'<br>';
						$picname_info=getimagesize($root_path_tag.$picname);
						$pic_size_th = $picname_info[0];
						$pic_size_eh = $picname_info[1];
 ...
 * @return    string
 */
function GetDDImgFromBodyForApp(&$body)
{
	global $forapp_dir_path,$root_path_tag;
	$litpic = '';
	preg_match_all("/(src)=[\"|'| ]{0,}([^>]*\.(gif|jpg|bmp|png))/isU",$body,$img_array);
	...
	if(count($img_array)>0)
	{
		foreach ($img_array as $k=>$v){
			...
			if(preg_match("#_lit\.#", $picname)){
				...
				if ($pic_size_th >480){
					if ($k==0){
						 GetDDImage('ddfirst', $picname,1,$pic_size_th,$pic_size_eh);
						 //$fu->copyFile(str_replace('.', '-lp',$root_path_tag.$picname), str_replace('uploads', 'upapp',str_replace('.', '-lp', $root_path_tag.$picname)),true);
					}
					$litpic = GetDDImageForApp('ddfirst', $picname,1,$pic_size_th,$pic_size_eh);
				}
				// else{
 ...
 *  获取等比 宽、高
 */
function getWthEht($th,$eh){
	global $cfg_ddimg_width,$cfg_ddimg_height;
	if ($th == 0 || $eh == 0){
		$return['w'] = $cfg_ddimg_width;
		$return['h'] = $cfg_ddimg_height;
	}else{
		$w = 480;
		$h = ($w/$th)*$eh;
		$return['w'] = $w;
 ...
 * @return    string
 */
function GetDDImageForApp($litpic, $picname, $isremote,$th=0,$eh=0)
{
	global $cuserLogin,$cfg_ddimg_width,$cfg_ddimg_height,$cfg_basedir,$ddcfg_image_dir,$cfg_addon_savetype;
	
	$getWthEht = getWthEht($th,$eh);
	$cfg_ddimg_width = $getWthEht['w'];
	$cfg_ddimg_height = $getWthEht['h'];
	...
	else
	{
		...
		else
		{
			if($litpic=='ddfirst' && !preg_match("#^http:\/\/#i", $picname))
			{
				...
				// $litpic = str_replace('uploads', 'upapp', $picname);
				$litpic = $picname;
			...
			{
				// $litpic = $picname;
				// $app_dir_path =  dirname(str_replace('uploads', 'upapp',$cfg_basedir.$litpic));
				// if (!is_dir($app_dir_path)){
					// $fu = new FileUtil();
					// $fu->copyFile($cfg_basedir.$litpic, str_replace('uploads', 'upapp', $cfg_basedir.$litpic),true);
					// $fu->unlinkFile(str_replace('uploads', 'upapp', $cfg_basedir.$litpic));
				// }
				// @ImageResize($cfg_basedir.$litpic,$cfg_ddimg_width,$cfg_ddimg_height,str_replace('uploads', 'upapp',$cfg_basedir.$litpic));
				return $picname;
			}
		}
	}
 ...
 * @return    string
 */
function GetDDImage($litpic, $picname, $isremote ,$th=0,$eh=0)
{
    global $cuserLogin,$cfg_ddimg_width,$cfg_ddimg_height,$cfg_basedir,$ddcfg_image_dir,$cfg_addon_savetype,$forapp_dir_path,$root_path_tag;
...
function AnalyseHtmlBody($body,&$description,&$litpic,&$keywords,$dtype='')
{
	...
	$body = str_replace('html.joyme.preview', 'html.joyme.com', $body);
 ...
 * @return    string
 */
function GetUpdateTest($update_type='', $joymearctypes='')
{
	$up_type = '';
	if($update_type){
		$up_type = '&up_type='.$update_type;
	}
        $up_othertypes = '';
        if($joymearctypes){
            $up_othertypes = '&up_othertypes='.$joymearctypes;
        }
    ...
    if($cfg_makeindex=='Y' || $cfg_make_andcat=='Y' || $cfg_make_prenext=='Y')
    {
        ...
        $jumpUrl = "task_do.php?typeid={$typeid}".$up_type.$up_othertypes."&aid={$arcID}&dopost={$dolists[0]}&nextdo=".preg_replace("#".$dolists[0]."[,]{0,1}#", '', $dolist);
