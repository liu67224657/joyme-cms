...
function GetOptionList($selid=0, $userCatalog=0, $channeltype=0)
{
    ...
    {
        $sonCats = '';
        LogicGetOptionArray($row->id, '─', $channeltype, $dsql, $sonCats, $selid);
        if($sonCats != '')
        {
            if($row->ispart==1 && $selid!=$row->id) $OptionArrayList .= "<option value='".$row->id."' class='option1'>".$row->typename."(封面频道)</option>\r\n";
            else if($row->ispart==2) $OptionArrayList .= '';
            else if( empty($channeltype) && $row->ispart != 0  && $selid!=$row->id) $OptionArrayList .= "<option value='".$row->id."' class='option2'>".$row->typename."(".$channels[$row->channeltype].")</option>\r\n";
            else if($selid!=$row->id) $OptionArrayList .= "<option value='".$row->id."' class='option3'>".$row->typename."</option>\r\n";
            $OptionArrayList .= $sonCats;
        }
        else
        {
            if($row->ispart==0 && (!empty($channeltype) && $row->channeltype == $channeltype)  && $selid!=$row->id)
            {
                $OptionArrayList .= "<option value='".$row->id."' class='option3'>".$row->typename."</option>\r\n";
            } else if($row->ispart==0 && empty($channeltype)  && $selid!=$row->id)
            { 
                // 专题
                $OptionArrayList .= "<option value='".$row->id."' class='option3'>".$row->typename."</option>\r\n";
            }
...
function LogicGetOptionArray($id,$step,$channeltype,&$dsql, &$sonCats, $selid=0)
{
    ...
    while($row=$dsql->GetObject($id))
    {
        ...
        if($row->channeltype==$channeltype && $row->ispart==1  && $selid!=$row->id)
        {
            $sonCats .= "<option value='".$row->id."' class='option1'>$step".$row->typename."</option>\r\n";
        }
        else if( (($row->channeltype==$channeltype && $row->ispart==0) || empty($channeltype))  && $selid!=$row->id)
        {
            $sonCats .= "<option value='".$row->id."' class='option3'>$step".$row->typename."</option>\r\n";
        }
        LogicGetOptionArray($row->id,$step.'─',$channeltype,$dsql, $sonCats, $selid);
    }
}