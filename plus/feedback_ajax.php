...
function GetList($page=1)
{
	...
	$lou = (empty($row['dd']) ? 0 : $row['dd']);  //解释：定义变量lou=所有留言总数
    ...
    while($fields = $dsql->GetArray('fb'))
    {
		...
		/*楼层号码*/
		  $n=$lou--; //让变量$n=所有留言总数递减 
				$curpage = empty($page) ? 1 : intval($page); //调出$curpage= 当前第几页 
		  $x=$n-($curpage-1)*$pagesize; //让变量$x=变量$n-(当前页-1)×每页显示留言数 
		  if ($x==1){ 
		   $floor = "沙发" ; 
		   } 
			 else if ($x==2){ 
...
<div class="decmt-box2">
  <ul>
    <li> <a href='<?php echo $spaceurl; ?>' class='plpic'><img src='<?php echo $mface;?>'  height='40' width='40'/></a> <span class="title"><a href="<?php echo $spaceurl; ?>"><?php echo $username; ?></a></span>
      <div class="comment_act"><span class="fl"></span></div>
      <div style="clear:both"><?php echo ubb($msg); ?> </div>
      <div class="newcomment_act"><span class="fr">
	  <span id='goodfb<?php echo $id; ?>'> </span> 
	  <span id='badfb<?php echo $id; ?>'> 
	  <span class="cmt_floor"> <?php echo $floor; ?> &nbsp;&nbsp;</span>
	  </span> 
